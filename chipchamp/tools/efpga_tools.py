...
def _ensure_tb(proj: str, base: str) -> str | None:
    """Per-design testbench, generated by name substitution from the
    template — the two shipped TBs differ only in the design name, and the
    template's structure (raw RTL as gold vs programmed fabric as DUT,
    !== compare, $fatal on divergence) is exactly what a cosim needs."""
    tb = os.path.join(proj, "Test", f"{base}_tb.v")
    if os.path.isfile(tb):
        return tb
    tpl = os.path.join(proj, "Test", "sequential_16bit_en_tb.v")
    if not os.path.isfile(tpl):
        return None
    with open(tb, "w") as fh:
        fh.write(open(tpl).read().replace("sequential_16bit_en", base))
    return tb


@tool("efpga-fabulous.simulate",
      "Prove the BITSTREAM behaves like the RTL: simulate the programmed "
      "fabric (bitstream loaded into the config chain) against the raw user "
      "design as a cycle-by-cycle gold model. A pass is behavioral "
      "bitstream-vs-RTL equivalence.", cost="metered", permission="submit",
      group="efpga-fabulous",
      schema={"type": "object", "properties": {
          "design": {"type": "string", "description":
                     "design path relative to the project, e.g. "
                     "user_design/foo.v — its bitstream must exist"},
          "fmt": {"type": "string", "default": "vcd",
                  "description": "waveform format: vcd | fst"},
          "project": {"type": "string"}},
          "required": ["design"]})
def efpga_simulate(ctx: ToolContext, design: str, fmt: str = "vcd",
                   project: str = "") -> dict:
    # argument teaching before availability, like the bitstream tool: what
    # is wrong with the CALL is knowable without any toolchain
    proj = _project(ctx, project)
    base = os.path.splitext(os.path.basename(design))[0]
    bin_rel = f"user_design/{base}.bin"
    if not os.path.isfile(os.path.join(proj, bin_rel)):
        return {"error": f"no bitstream at {bin_rel} — run "
                         f"efpga-fabulous.bitstream for this design first; "
                         f"the cosim programs the fabric with that file."}
    if _ensure_tb(proj, base) is None:
        return {"error": "no Test/ template testbench in this project"}
    adapter = ctx.registry.for_role("efpga-fabulous")
    if not adapter or not adapter.available():
        return {"error": "FABulous not available"}
    plan = adapter.simulate(proj, fmt=fmt, bitstream=bin_rel)
    rec, res = ctx.submit(plan, adapter,
                          input_files=_efpga_inputs(proj, design)
                          + [os.path.join(proj, bin_rel)], timeout=1800)
    return {"job": rec.id, "status": rec.status,
            "sim_passed": res.metrics.get("sim_passed"),
            "waveform": (ctx.rel(res.artifacts["waveform"])
                         if res.artifacts.get("waveform") else None),
            "summary": res.summary}
